...
class intelx:
    ...
    def FILE_PREVIEW(self, ctype, mediatype, format, sid, bucket='', e=0, lines=8):
        ...
        """
        time.sleep(self.API_RATE_LIMIT)
        r = requests.get(f"{self.API_ROOT}/file/preview?sid={sid}&f={format}&l={lines}&c={ctype}&m={mediatype}&b={bucket}&k={self.API_KEY}")
    ...
    def FILE_TREE_VIEW(self, indexfile:str, bucket:str):
        """
        Show a treeview of an item that has multiple files/folders
        """
        time.sleep(self.API_RATE_LIMIT)
        try:
            r = requests.get(f"{self.API_ROOT}/file/view?f=12&storageid={indexfile}&bucket={bucket}&k={self.API_KEY}", timeout=5)
    ...
    def selectors(self, systemid:str, bucket:str):
        """
        List content of the file
        """
        time.sleep(self.API_RATE_LIMIT)
        r = requests.get(self.API_ROOT + f'/item/selector/list/human?id={systemid}&bucket={bucket}&k={self.API_KEY}')
        if r.status_code == 200:
    ...
    def statistics(self, id):
        """
        ID from the query /search
        """
        time.sleep(self.API_RATE_LIMIT)
        r = requests.get(self.API_ROOT + f'/intelligent/search/statistic?id={id}&k={self.API_KEY}')
        return r.json()